...
export default function EditReview(props) {
	...
	const [editReviewResponseMessage, setEditReviewResponseMessage] =
		useState("");
	...
	async function editReview(editedReview) {
		try {
			...
			setEditReviewResponseMessage(
				`Review of ${review?.email} successfully updated`
			);
		...
		<AdminMainWrapper sx={{ p: 4 }}>
			<InputBox>
				<form onSubmit={onEditReview}>
					<Stack spacing={2}>
						<Box>
							<InfoHolder>
								...
								<Typography sx={{ color: "gray", fontSize: 12 }}>
									on {formatDate(review?.createdOn)}
								</Typography>
							</InfoHolder>
						</Box>
						<Box sx={{ display: "flex", gap: 1 }}>
							...
							/>{" "}
							<Typography>
								{review?.ratingValue ? `(${review?.ratingValue} stars)` : <></>}
							</Typography>
						</Box>
						<UniversalInput
							inputRef={contentRef}
				...
				{editReviewResponseMessage ? (
					<Zoom in={editReviewResponseMessage.length > 0 ? true : false}>
				...
				{editReviewErrorMessage ? (
					<Zoom in={editReviewErrorMessage.length > 0 ? true : false}>
						<Alert
